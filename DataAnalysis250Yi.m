function [DataExcelFinal,ProcessedDataAvg] = DataAnalysis250Yi(data,Stim,Fs,L,Prestim,PrestimWindow,EntireResponse,StimType)
...
ER = round(Fs*Prestim) + round(Fs*(EntireResponse));  % Entire Response (10~260 ms)
% F0 tracking parameters:
window_size = 0.04; % 40 ms
step_size = 0.01;   % 10 ms
start_time = 0.01;  % 10 ms
end_time = 0.26;    % 260 ms
...
%     xlabel('Time (ms)');     
    ...
    xlim([-40 250]);    ylim([-0.3 0.3]); title([char(Initials(i)),': SNR (dB) = ', num2str(ERDataAlldBSNR(i),'%.3f')]);    
    ...
    % F0 tracking:
    [F0_contour_stimulus,correlation_scores_stimulus,] = F0tracking(Stim,Fs,window_size,step_size,start_time,end_time,lags,F0_lag_range,'noplot',subplot_pos,ones(1,22));
    [F0_contour_FFR,correlation_scores_FFR,F0_tracking(i)] = F0tracking(DataAvg,Fs,window_size,step_size,start_time,end_time,lags,F0_lag_range,'plot',subplot_pos,F0_contour_stimulus);
%     F0_tracking_accuracy(i) = corr(F0_contour_stimulus', F0_contour_FFR');

    % Cross-Correlation (R-S):
    [Datac,Datalags,DataCmax(i),DataCmaxLag(i)] = FFRsrcorr(filteredStim,DataAvg,Prestim,0.000,0.250,0.015,Fs);
...
DataExcel = num2cell([ERDataRMSNoise;ERDataRMSSignal;ERDataAllSNR;CCACscore;F0_tracking;DataCmaxLag;fisher(DataCmax)]);
